...
5. **抖音视频必读 `references/short-video-copywriting.md`**：按抖音优先的前3秒/前5秒、口播节奏、字幕画面、标题话题、评论 CTA、情感合规和发布后复盘规范执行；不把固定“3秒/完播率”当作硬规则。
6. **检索 `assets/hooks/` 标题库/钩子库**（按选题类型挑 1-2 个参考），读取 `assets/cases/index.md` 选 1-2 个最贴近的博主，读其 `assets/cases/<博主名>.md` 挑 2-3 条案例做风格示例。若库为空，仅依人设 + 平台规则生成。
7. 读取 `references/frameworks.md` 中匹配博主的选题方向/框架 → 纳入。
8. 按 `references/platform-rules.md` + `style-guide.md` 生成 **3-5 条不同角度**的完整文案。
9. 每条按 style-guide 第七节格式交付（标题/形式/主要观点/口播脚本/话题标签）。
10. 按 `references/originality.md` 自检 + `scripts/similarity.py`，不通过则改写。
11. **发布前总检**：运行 `python scripts/prepublish_check.py --file <文案> --platform <平台> --title "<标题>" --refs <案例库目录>`（AI味+违禁词+字数+标题+洗稿），不通过则改写后重检。
...
- **抖音口播技巧（抖音必读）: `references/short-video-copywriting.md`**
